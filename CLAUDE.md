...
Subagents are used only for three sanctioned cases: read-only research (an existing codebase, or a sibling project this Skill already planned), phase 17's mechanical audit (`validate-ids.mjs`/`validate-traceability.mjs`, executed and reported by a subagent while the semantic conflict scan — do any two approved documents actually contradict each other? — stays with the main thread, since that needs the full history of confirmed decisions a subagent doesn't have), and the full compatibility audit's judgment-criteria review (`rules/skill-drift.md`, one already-approved phase per subagent). A subagent never runs the confirmation loop and never decides anything — for the judgment-review case specifically, the main thread also independently re-verifies every reported finding before treating it as real, since a subagent can misread ambiguous prose.

### Versioning and drift (`rules/skill-drift.md`, `rules/versioning.md`)

`SKILL.md`'s `version:` frontmatter follows Patch (wording/tooling fixes only) / Minor (new mandatory question, gate criterion, or template field that could make an approved doc incomplete) / Major (reserved, unused). A target project's `project-state.md` records the `skill_version` it was last touched with; on resume, a Patch-only gap is a brief mention, a Minor-or-above gap is an explicit blocking question with three answers — continue normally, run `audit-compatibility.mjs`'s scriptable-only audit, or run the full audit (same script's findings plus a per-phase subagent judgment-criteria review) — never a silent, automatic rewrite of an approved document. `export_labels` backfill piggybacks on this same drift check but is never itself a question — it's applied mechanically whenever drift is detected and the project's language isn't English.
