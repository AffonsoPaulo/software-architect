...
- **Malformed YAML**: the front-matter parser (`scripts/lib/yaml-lite.mjs`) is a deliberately minimal subset parser, not a general YAML implementation — see its file header for exactly what it supports. If `docs/project-state.md` uses YAML features outside that subset (multi-line block scalars, anchors), it will parse incorrectly or silently produce unexpected structure. Keep edits to the same shape the templates use.
...
- **A field name doesn't match the template**: the scripts extract artifacts and `traces_to` generically by walking the parsed front-matter — they only work if a document actually uses the field names its `templates/*.md` file specifies (`traces_to`, not a differently-named field). This bit the Skill's own construction once (see `plan-23`'s commit history: `templates/requirements.md` briefly used `business_rule` instead of `traces_to` and was invisible to the extractor as a result) — if a document was hand-written or generated inconsistently with its template, this is the first thing to check.
- **A genuinely non-artifact `id` field looks like a violation**: only uppercase-prefixed `id` values (e.g. `REQ-001`) are treated as artifacts; intentional slugs like a milestone's `id: "milestone-mvp"` or a screen's `id: "screen-task-board"` are deliberately excluded (see `scripts/lib/docs.mjs`'s `ID_RE` comment). If something that should be excluded is being picked up, check it isn't accidentally using an uppercase-prefix-shaped id.
...
Editing an approved document's file directly, outside of a CR, will desynchronize it from `project-state.md`'s version tracking and from anything downstream that traces to it — don't do this even if it seems faster.
