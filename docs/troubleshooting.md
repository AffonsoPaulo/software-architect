...
- **A genuinely non-artifact heading looks like a violation**: only headings whose first token matches an uppercase prefix (e.g. `REQ-001`) are treated as artifacts; a milestone's or screen's plain-name heading (e.g. `### Recipe List / Home`) is correctly excluded from ID checks even though it can still carry a `Traces to` metadata line and count as a reference (see `scripts/lib/docs.mjs`'s `HEADING_ID_RE`/`prefixOf`). If something that should be excluded is being picked up, check its heading doesn't accidentally start with an uppercase-prefix-shaped token.
