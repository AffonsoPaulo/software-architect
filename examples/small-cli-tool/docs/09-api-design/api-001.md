...
*Traces to: UC-001, ARCH-001 · Author: Sam*

| Field | Value |
|---|---|
| Trigger | `convert <input-file> --to <format> [--from <format>] [--output <path>]` |
| Input | input-file: path to an existing, readable file. `--to`: target format (csv or json). `--from`: optional, inferred from extension if omitted. `--output`: optional, defaults to input filename with the target extension. |
