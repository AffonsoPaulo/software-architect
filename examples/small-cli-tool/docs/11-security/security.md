...
No authentication mechanism — this is a local CLI tool with no network surface. Authorization is entirely delegated to the operating system's file permissions on the input/output paths, confirmed given the tool's local-only scope.
...
## Compliance
None applicable — no PII, no regulated data category, purely a local format converter.
