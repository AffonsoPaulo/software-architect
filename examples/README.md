...
Each example includes its final `project-state.md`, every document produced (plain markdown, per `rules/document-format.md` — no YAML front-matter except `project-state.md` itself), a `transcript.md` with real dialogue excerpts, and a clean run of `scripts/self-test.mjs`.
