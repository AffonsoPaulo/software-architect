...
## Scriptable criteria
- [ ] "Has real dates" is set explicitly (not null/missing) — checked by `scripts/validate-gate.mjs`
...
- [ ] "Has real dates" reflects an actual confirmed commitment from the user, not an aspiration or a passing mention
- [ ] `gantt` is used only if "Has real dates" is true; otherwise a dependency `flowchart` is used
