...
- Once language is confirmed (and isn't `"en"`): also translate and record `project-state.md`'s `export_labels` (`table_of_contents`, `change_requests`, `mermaid_fallback`, `project_documentation`, `copy_markdown`, `toc_instructions`, `shell_table_field`, `shell_table_value`) in that same language — `scripts/build-doc-site.mjs`/`scripts/build-doc-word.mjs`'s own fixed chrome text, which otherwise has no document of its own to read a translation from the way every phase's own title does. Not a separate question to the user — a direct, mechanical consequence of the language answer just confirmed, same translation quality expected everywhere else in the document. `shell_table_field`/`shell_table_value` is also the exact pair to write into every `| Field | Value |` shell table this project produces from here on (`rules/document-format.md`) — not retranslated fresh each time.
- Author → this cycle's entry in `project-state.md`'s `cycles[].author`, plus the `Author` row in `calibration.md`'s table (`rules/versioning.md`). Once this phase's gate passes, it also becomes the `Author` on every artifact this cycle goes on to create, and the first `docs/CHANGELOG.md` entry ("Initial calibration confirmed", or for incremental mode, "Cycle N calibration confirmed — <scope>") is logged with it. For incremental mode specifically, this is also the entry that resets `docs_version` to `<cycle id>.0.0` (`rules/versioning.md`) — Major becomes this cycle's own id, Minor and Patch both reset to 0, rather than continuing the previous cycle's Minor count.
...
- Hand-editing `cycles[]`/`active_cycle_id` for a new incremental cycle instead of running `scripts/new-cycle.mjs` — hand-editing risks exactly the mistakes that script exists to prevent (wrong next id, a forgotten field, an accidentally reset `id_sequences`).
