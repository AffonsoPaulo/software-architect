...
- What do loading, empty, and error states look like by default, across screens? (Recorded once as the project's general convention — a specific screen only needs its own answer when it genuinely differs.)
- What are the actual responsive breakpoint values used project-wide?
- Are user actions tracked at all, and under what general naming convention? (Same "record once, diverge per screen only when needed" treatment as states above.)
