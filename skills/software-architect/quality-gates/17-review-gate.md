...
- [ ] `scripts/validate-tone.mjs` reports zero violations: no document references this Skill's internal phase numbers, leaves a confirmation-marker bracket in written text, cites a `rules/`/`playbooks/`/`templates/`/`scripts/` path, or echoes the Skill's own policy language (`rules/document-format.md`'s "Never let the Skill's own process show through")
