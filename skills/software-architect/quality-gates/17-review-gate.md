...
- [ ] `scripts/validate-heading-language.mjs` reports zero violations (skipped entirely when `language: "en"`): no phase document's fixed section headings, table header row, `**Bold label**` sub-section, or inline `**Label**: value` field is still the template's literal English text when the project's confirmed language is something else (`rules/language-policy.md`)
