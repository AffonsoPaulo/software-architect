...
export function buildDocWord(projectRoot, outputPath) {
  ...
  // Document control line, per rules/document-format.md's "Never let
  // the Skill's own process show through": version and freshness are
  // what a real document's own front matter carries, not which tool
  // produced it. Plain data (no connecting prose) deliberately, so it
  // needs no translated label of its own the way "Table of Contents" does.
  ...
  parts.push(
    `{\\pard\\qc\\sa240\\b\\fs24 ${escapeRtfText(tocLabel)}\\b0\\par}\n` +
  ...
  for (const phase of EXPORTABLE_PHASES) {
    ...
    parts.push(renderFiles(numberedFiles));
    phasesRendered++;
  }
  ...
  if (changelogFile) {
    parts.push(PAGE_BREAK);
    chapterNum++;
    parts.push(renderFiles([{ ...changelogFile, isMain: true, content: prefixChapterHeading(changelogFile.content, chapterNum) }]));
    phasesRendered++;
  }
