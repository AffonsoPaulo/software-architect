...
import { EXPORTABLE_PHASES, prefixChapterHeading, collectPhaseFiles, collectChangeRequestFiles, readChangelogFile } from './lib/doc-tree.mjs';
...
const PAGE_TEMPLATE = (title, navHtml, contentHtml, mermaidJs, pageHeader) => `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<main>
${pageHeader ? `<div class="page-header">${pageHeader}</div>` : ''}
${contentHtml}
</main>
<script>
...
// `mainTitle` already carries its chapter number (renderPhase bakes it
// in via prefixChapterHeading before rendering, using this document's
// own translated title) — this is only a defensive fallback for the
// edge case of a phase directory that exists but has no title heading
// yet. It falls back to phase.title's own concept name with its
// internal phase-number prefix stripped, since that fixed phase number
// doesn't match this export's own sequential chapter count.
function resolveNavTitle(phase, mainTitle) {
  if (mainTitle) return mainTitle;
  return phase.title.replace(/^\d+\s*·\s*/, '');
}
...
export function buildDocSite(projectRoot, outputPath) {
  ...
  if (changelogRendered) {
    ...
    // bakes it in the same way renderPhase does) — the fallback still
    // needs one, since it never went through that same prefixing.
    navParts.push(buildNav('changelog', changelogRendered.mainTitle || `${chapterNum} — Changelog`, []));
  ...
  // front matter carries, not which tool produced it. Plain data (no
  // connecting prose), so it needs no translated label of its own.
  const changelog = (projectState && Array.isArray(projectState.changelog)) ? projectState.changelog : [];
  const lastUpdated = changelog.length > 0 ? changelog[changelog.length - 1].date : null;
  const controlBits = [];
  if (projectState && projectState.docs_version) controlBits.push(`v${projectState.docs_version}`);
  if (projectState && projectState.language) controlBits.push(projectState.language);
  if (lastUpdated) controlBits.push(lastUpdated);
  const pageHeader = controlBits.length > 0 ? controlBits.map(escapeHtml).join(' &middot; ') : '';

  const html = PAGE_TEMPLATE(title, navParts.join('\n'), contentParts.join('\n'), MERMAID_JS, pageHeader);
