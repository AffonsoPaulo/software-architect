...
- **A `| Field | Value |` shell table**, when an artifact has several single-value facts worth consolidating into one spec-sheet block (`templates/requirements.md`'s Rationale/Source/Risk if not met/Verification method/Dependencies; `templates/use-cases.md`'s Goal in context/Scope/actors/Trigger/etc.; `templates/api.md`'s Trigger/Input/Effect-output/etc.) — the standard Volere-shell/IEEE-830 convention for a requirement or use-case record. A field with genuinely list-shaped content (`Acceptance criteria`, `Main flow`) never goes in this table, even if it would technically fit in a cell — it stays its own `**Label**` + list. Both exporters (`scripts/build-doc-site.mjs`, `scripts/build-doc-word.mjs`) recognize this exact header pair and render it without the header row, first column styled as a row label instead — the classic headerless shell look, without giving up a valid, portable Markdown table in the source file itself. This is why the pair is `export_labels.shell_table_field`/`shell_table_value` (`templates/project-state.md`), confirmed once and reused verbatim in every shell table across the project, rather than retranslated fresh each time — the exporters match it by exact text, so an inconsistent translation would just mean an inconsistent handful of tables lose the treatment, not an error.
...
A table's own column headers (`Step`/`Condition`/`Result`, `Attribute`/`Meaning`, and so on) translate along with the rest of the document, the same as a section heading — see `rules/language-policy.md`. Only the fixed enum values a cell might contain (a `Status` column's `Open`/`Closed`) stay untranslated, not the header naming that column. `Field`/`Value` is the one exception among headers: it still translates, but to the one confirmed `export_labels.shell_table_field`/`shell_table_value` pair (above), not freely per occurrence.
