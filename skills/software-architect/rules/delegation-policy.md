...
1. **Read-only brownfield research** (phases 00, 08, 09 — Calibration, Architecture, API Design): when the project is brownfield, a subagent explores the existing code/schema/API and reports **facts**, never decisions, back to the main thread. The main thread uses those facts to ask better-informed questions — it does not skip asking.
